import { useEffect, useState } from "react";
import { format, parseISO } from "date-fns";
import { es } from "date-fns/locale";
import { useAgenda } from "../features/agenda/useAgenda.js";
import { useReveal } from "../hooks/useReveal.js";
import { SectionDivider } from "./SectionDivider.jsx";

function capitalize(text) {
  return text.charAt(0).toUpperCase() + text.slice(1);
}

function formatearFecha(isoDate) {
  const fecha = parseISO(isoDate);
  return `${format(fecha, "dd")}/${capitalize(format(fecha, "MMMM", { locale: es }))}/${format(fecha, "yyyy")}`;
}

const today = format(new Date(), "yyyy-MM-dd");

export function HireSection({ selectedDate }) {
  const { busyDates } = useAgenda();
  const [nombreContratante, setNombreContratante] = useState("");
  const [fechaContratacion, setFechaContratacion] = useState("");
  const [horasRequeridas, setHorasRequeridas] = useState("");
  const root = useReveal();

  useEffect(() => {
    if (selectedDate) setFechaContratacion(selectedDate);
  }, [selectedDate]);

  const isBusy = Boolean(fechaContratacion) && busyDates.has(fechaContratacion);

  const handleContratacionSubmit = (e) => {
    e.preventDefault();

    if (!nombreContratante || !fechaContratacion || !horasRequeridas) {
      alert("Por favor, completa todos los campos");
      return;
    }

    if (busyDates.has(fechaContratacion)) {
      alert(
        "Esa fecha ya está ocupada. Elegí otro día en el calendario de disponibilidad.",
      );
      return;
    }

    const diaSemana = capitalize(
      format(parseISO(fechaContratacion), "EEEE", { locale: es }),
    );
    const fechaFormateada = formatearFecha(fechaContratacion);

    const mensaje = `📅 *SOLICITUD DE CONTRATACIÓN* 📅

*Nombre:* ${nombreContratante}
*Día:* ${diaSemana}
*Fecha:* ${fechaFormateada}
*Horas requeridas:* ${horasRequeridas} hora(s)

¡Espero su confirmación!`;

    const whatsappUrl = `https://wa.me/523313023706?text=${encodeURIComponent(mensaje)}`;
    window.open(whatsappUrl, "_blank");

    setNombreContratante("");
    setFechaContratacion("");
    setHorasRequeridas("");
  };

  return (
    <section
      id="contratar"
      ref={root}
      className="scroll-mt-[72px] py-24 px-6 bg-gradient-to-r from-jade-900/50 via-ink-950 to-jade-900/50"
    >
      <SectionDivider />
      <div className="max-w-xl mx-auto">
        <div className="text-center mb-10">
          <div className="w-16 h-16 mx-auto mb-6 bg-jade-600 rounded-full flex items-center justify-center">
            <svg
              className="w-8 h-8"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"
              />
            </svg>
          </div>
          <h2 className="text-4xl font-bold mb-4">¡Contrátanos!</h2>
          <p className="text-ivory-muted">
            Llena el formulario y nos pondremos en contacto contigo para
            agendar tu evento. ¡La mejor música norteña te espera!
          </p>
        </div>

        <form
          onSubmit={handleContratacionSubmit}
          className="bg-ink-800/60 backdrop-blur-sm rounded-2xl p-8 shadow-2xl border border-jade-700/40"
        >
          <div className="mb-6">
            <label className="block text-ivory font-semibold mb-2">
              Nombre de quien contrata
            </label>
            <input
              type="text"
              value={nombreContratante}
              onChange={(e) => setNombreContratante(e.target.value)}
              placeholder="Su Nombre porfavor"
              className="w-full px-5 py-4 rounded-xl bg-ink-950 text-ivory border border-jade-700/60 focus:border-jade-400 focus:outline-none transition-colors placeholder-ivory-muted/60"
              required
            />
          </div>

          <div className="mb-6">
            <label className="block text-ivory font-semibold mb-2">
              Fecha de contratación
            </label>
            <input
              type="date"
              value={fechaContratacion}
              onChange={(e) => setFechaContratacion(e.target.value)}
              min={today}
              className="w-full px-5 py-4 rounded-xl bg-ink-950 text-ivory border border-jade-700/60 focus:border-jade-400 focus:outline-none transition-colors cursor-pointer"
              required
            />
            {fechaContratacion && !isBusy && (
              <p className="text-ivory-muted text-sm mt-2">
                Vas a agendar:{" "}
                <span className="text-ivory font-semibold">
                  {formatearFecha(fechaContratacion)}
                </span>
              </p>
            )}
            {isBusy && (
              <p className="text-gold-300 text-sm mt-2">
                Esa fecha ya está ocupada. Elegí otra en el calendario de
                arriba.
              </p>
            )}
          </div>

          <div className="mb-8">
            <label className="block text-ivory font-semibold mb-2">
              ¿Cuántas horas requiere?
            </label>
            <input
              type="number"
              value={horasRequeridas}
              onChange={(e) => setHorasRequeridas(e.target.value)}
              placeholder="Ej: 4"
              min="1"
              max="12"
              className="w-full px-5 py-4 rounded-xl bg-ink-950 text-ivory border border-jade-700/60 focus:border-jade-400 focus:outline-none transition-colors placeholder-ivory-muted/60"
              required
            />
          </div>

          <button
            type="submit"
            disabled={isBusy}
            className="w-full px-8 py-4 bg-jade-600 hover:bg-jade-500 disabled:opacity-50 disabled:cursor-not-allowed text-ivory font-bold rounded-xl transition-all transform hover:scale-[1.02] shadow-lg shadow-jade-600/40 flex items-center justify-center gap-3"
          >
            <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            Solicitar fecha
          </button>
        </form>
      </div>
    </section>
  );
}
